...
public class SpringConfig {
    ...
    @Bean//configuração inicial da segurança da aplicação
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        return http
                .csrf(AbstractHttpConfigurer::disable)
                .authorizeHttpRequests(authorize -> authorize.requestMatchers(HttpMethod.POST,"/token/login").permitAll())
                .authorizeHttpRequests(authorize -> authorize.requestMatchers(HttpMethod.POST,"/user/register").permitAll())
               // .authorizeHttpRequests(authorize -> authorize.requestMatchers(HttpMethod.GET,"user/list").hasRole(Role.Values.))
